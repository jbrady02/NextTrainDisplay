...
# NextTrainDisplay
A local web page that displays live information about the next trains to depart a given SEPTA station. For this, a looping Python program is used to take data from SEPTA’s API and output a HTML file containing information about the next trains to depart a given station.
# Description
...
First, open settings.txt. Make the first line the station that you want the train information for. Please note that SEPTA's API sometimes uses different names internally than what is displayed publicly. **If your display is not showing any trains, this may be the issue.** If you want to limit trains to a certain direction, add a second line and either 'N' or 'S'.\
